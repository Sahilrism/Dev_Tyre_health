...
def parse_args(argv=None):
    ...
    parser.add_argument('--claim_id', default=101, type=str,
                        help='Used for creating folder inside the server')

                        

    parser.add_argument('--unique_file_name', default="rotate", type=str,
                        help='Used for creating subfoler inside the folder on the server')
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                ...
                print("$$$",cfg.dataset.class_names[j])
                print("######",_class)
                pro_time=dict()
                pro_time["class"]=_class
                with open(args.claim_id + "/{}.json".format(args.unique_file_name), "w") as fout:
                   json.dump(pro_time, fout)
...
#AKSHAY Check here the path starts...
def evalimages(net:Yolact, input_image_path:str, output_folder:str):
    ...
    name=str(name.split("/")[-1].split(".")[0])+".png"
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        a=evaluate(net, dataset)
        print("AKSHAY INSIDE RETURN FROM EVALUATE",a)
        brand_final(a)
