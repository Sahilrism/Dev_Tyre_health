...
def parse_args(argv=None):
    ...
    parser.add_argument('--config', default="coin_resnet50_config",
...
def prep_display(name,claimid,dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    """
    ...
    with timer.env('Copy'):
        ...
        try:
            ...
            #     print("name =========== ",name)
            #     img = cv2.imread(f"{claimid}/Splitted_Images/"+f"{name}.jpg")
            ...
            if cfg.eval_mask_branch:
                masks = t[3][:5]
                masks_arr=masks.tolist()
                mask_array_merged = np.array(masks_arr).flatten().tolist()
                
                img_path = os.path.join(claimid, "Splitted_Images", f"{name}.jpg")
                ...
                y_coords = np.arange(height)
                X, Y = np.meshgrid(x_coords, y_coords)
                im_array = np.column_stack((X.ravel(), Y.ravel()))
                mask_array_merged = np.array(mask_array_merged)    
                cord_array = im_array[mask_array_merged == 1.0].tolist()
...
def evalimage(name,claimid,net:Yolact, path:str, save_path:str=None):
    ...
    img_numpy = prep_display(name,claimid,preds, frame, None, None, undo_transform=False)
...
def evalimages(claimid,net:Yolact, input_folder:str, output_folder:str):
    ...
    def tyre_mask_threads(path):
        name = path.split("/")[-1].split(".")[0]
        path = claimid + "/Splitted_Images/" +name+ ".jpg"
        out_path = claimid + "/Masked_Images/" +name+ ".png"
        mask_array = evalimage(name,claimid,net, path, out_path)
        tyre_mask_dict[name] = mask_array
        
    t1 = threading.Thread(target=tyre_mask_threads,args=(top_left,))
    t2 = threading.Thread(target=tyre_mask_threads,args=(top_right,))
    t3 = threading.Thread(target=tyre_mask_threads,args=(bottom_left,))
    t4 = threading.Thread(target=tyre_mask_threads,args=(bottom_right,))
    ...
    with open(claimid+"/tyre_mask.json", "w") as json_file:
            json.dump(tyre_mask_dict, json_file)
...
trained_model="app/tyre_mask.pth"
config="tyre_mask_resnet50_config"
...
with torch.no_grad():
    ...
    dataset = None        
    print('Loading tyre mask model...', end='')
    net = Yolact()
