...
def coin_detect(save_img=False):
    ...
    opt_weights =  "app/epoch_3499_10_01_23.pt"
...
def coin_prediction(lead_id):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if float(conf_)>= 0.10:
                        print("confidence > 10")
                        if ("10" not in str(label)) and (float(conf_)>=confprev):
                            ...
                            detected_box_dict[f'finger_{count}'] = [(x_,y_),(int(xyxy[2]),int(xyxy[3]))]
                            # print("This is finger box",detected_box_dict['finger'])
                            print("This is finger box22",detected_box_dict)
                    ...
                    if "10" in str(label):
                        ...
                        for k in range(len(det)):
                            ...
                            coin_x2 = int(xyxy[2])
                            coin_y2 = int(xyxy[3]) 
                            print("Coin ka x2 yaha print hua hai >>>>>>", coin_x2)  
                            print("Coin ka y2 yaha print hua hai >>>>>>", coin_y2)                             
                            ...
                            print("This is coin box22",detected_box_dict)
                            # if h<=w and met_f:
                            resolution_flag = False
                            met_f = False

                            # crop_img=img_[y-10:y+ h+30, x-10:x + w+20] 
                            crop_img=img_[y-5:y+ h+15, x-5:x + w+10]
        ...
        test_dict = {}
        keys = ["left_finger","right_finger","10","10_upper"]
        for i in detected_box_dict.keys():
            if i not in keys:
                test_dict[i] = detected_box_dict[i]

        holding_hand = ''
        hand = ''
        if len(test_dict)>0:
                
            coin_box = detected_box_dict['10_upper']
            coin_box_center = abs(coin_box[0][0] + coin_box[1][0])//2
            print('COIN CENTER >>>>', coin_box_center)
            holding_hand = ""
            holding_location = ""

            for i in test_dict.keys():
                finger_dict = test_dict[i]
                center = abs(finger_dict[0][0]+finger_dict[1][0]/2)
                print(test_dict[i])
                print(center)
                hand = ""
                
                if center > coin_box_center:
                    hand = "right"
                else:
                    hand = "left"
                ...
                print("Hand >>>",hand)
                print("X2 >>",finger_dict[1][0])

                if hand == "left" and finger_dict[1][0] > coin_box_center:
                    holding_location = 'top'

                elif hand =="right" and finger_dict[0][0] < coin_box_center:
                    holding_location = 'top'
                
                else:
                    holding_location = "side"
                    
                print("Check Hold >>>",holding_location)
                if holding_location == "side" and hand == "left":
                    if finger_dict[1][0] > coin_box[0][0]:
                        holding_hand = "Normal"
                        detected_box_dict['finger'] = test_dict[i]
                        keys.append('finger')
                        break
                        
                    
                elif holding_location == "side" and hand == "right":
                    if finger_dict[0][0] < coin_box[1][0]:
                        holding_hand = "Normal"    
                        detected_box_dict['finger'] = test_dict[i]
                        keys.append('finger')
                        break
                        
                elif holding_location == "top":
                    detected_box_dict['finger'] = test_dict[i]
                    holding_hand = "Upper"
                    keys.append('finger')
                    
            del_key = test_dict.keys()  
            print(del_key)
            for j in del_key:
                del detected_box_dict[j]

            print("Hand location >>>",holding_hand)    
            print("Final dict >>>", detected_box_dict)

        else:
            holding_hand = "Not Detected" 
        ...
        # coin_center_x = (detected_box_dict['10'][0][0]+ coin_x2)/2
        # coin_center_y = (detected_box_dict['10'][0][1] + coin_y2)/2
        # hand_location = "Normal"
        # print("*"*50)
    ...
    # ########################################################## 
        ...
        return resolution_flag,detected_box_dict,holding_hand,finger_det
