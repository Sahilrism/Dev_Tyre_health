...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='app/coin_mask.pth', type=str,
                        help='Trained state_dict file path to open. If "interrupt", this will open the interrupt file.')
    parser.add_argument('--top_k', default=5, type=int,
                        help='Further restrict the number of predictions to parse')
    parser.add_argument('--claim_id', default='/1234', type=str,
                    help='Folder To save predicted image and json')
    ...
    parser.add_argument('--config', default="coin_10_mask_config",
...
def prep_display(claimid,dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    print("pred_display called")
    
    #####################################################################
    # print("dets_out >>>>>>>>> ",dets_out[0]["detection"])
    # bounding_box = dets_out[0]["detection"]["box"].tolist()
    ...
    with timer.env('Copy'):
        ...
        #### converting masked output into co-ordinates
        cord_array = []
        try:
            if cfg.eval_mask_branch:
                ...
                masks_arr=masks.tolist()
                # print(masks_arr.shape[1:])
                # masks_arr=masks_arr.astype(int)
                # print("masks_arr >>>>>> ",masks_arr)
            
                import torch,codecs
                # print(img)
                # # Assuming the input image has shape (H, W)
                # H, W,dim = img.shape
                # # Assuming the `mask` tensor is provided as a 2D tensor with shape (H, W)
                # mask = all_objects_mask
                # # Convert the mask tensor to a binary mask
                # binary_mask = (mask >= 0).int()
                # # Get the indices of all non-zero elements in the binary mask
                # indices = binary_mask.nonzero()
                # # Convert the indices to pixel coordinates
                # pixel_coords = indices.float() * torch.tensor([W, H])
                # # Print the pixel coordinates
                # # print("pixel_coords >>>>>>>>>>> ",pixel_coords)
                # b = masks_arr.tolist() # nested lists with same data, indices
                # print("len masks_arr ",len(b[0]))
                # file_path = "/home/deep/Documents/coin_training/coin_masking/yolact/test_output/masked_array.json" ## your path variable
                ...
                # file_path = "/home/deep/Documents/coin_training/coin_masking/yolact/test_output/masked_array.json"
                # obj_text = codecs.open(file_path, 'r', encoding='utf-8').read()
                # b_new = json.loads(obj_text)

                mask_array = masks_arr
                # print("mask_array",mask_array)
                mask_array_merged = []
                # for list1 in mask_array:
                #     for list2 in list1:
                #         mask_array_merged.extend(list2)
                mask_array_merged = np.array(mask_array).flatten().tolist()
                print(len(mask_array_merged))
                # read the original image
                img = cv2.imread(glob.glob(f"{claimid}/"+"*.jpg")[0])
                ...
                mask_array = np.array(mask_array_merged)
                im_array = np.array(im_array)
                cord_array = im_array[mask_array == 1].tolist()
                # for i in range(len(mask_array_merged)):
                #     if int(mask_array_merged[i])==1:
                #         cord_array.append(im_array[i])
                file_path = f"{claimid}/mask_coord.json" ## your path variable
                json.dump(cord_array, codecs.open(file_path, 'w', encoding='utf-8'), 
                        separators=(',', ':'), 
                        sort_keys=True, 
                        indent=4)
                print("json dumped and saved")
...
def evalimage(claimid,net:Yolact, path:str, save_path:str=None):
    print("evalimage called")
    print("claimid >>> ",claimid)
    print("original img path >>> ",path)
    # path = claimid + "image.jpg"
    # save_path = claimid + "image.png"
    frame = torch.from_numpy(cv2.imread(path)).cuda().float()
    # frame = cv2.imread(path)
    # print("original img array >>> ",frame)
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch)

    img_numpy = prep_display(claimid,preds, frame, None, None, undo_transform=False)
    # cv2.imwrite(save_path, img_numpy)
    
    # from PIL import Image, ImageDraw
    # image = Image.open(claimid + "/image.jpg")
    # draw = ImageDraw.Draw(image)
    # for point in img_numpy:
    #     draw.point(point, fill="red")
    #     # Save the image
    #     image.save(claimid + "/image_predicted.jpg")
...
def evalimages(claimid,net:Yolact, input_folder:str, output_folder:str):
    if not os.path.exists(output_folder):
        os.mkdir(output_folder)

    print("evalimages called")
    ...
    #     img_numpy = evalimage(claimid,net, path, out_path)
    #     print(path + ' -> ' + out_path)
    path = claimid + "/image.jpg"
    out_path = claimid + "/image.png"
    evalimage(claimid,net, path, out_path)
    print('Done.')
...
def evalvideo(net:Yolact, path:str, out_path:str=None):
    ...
    def prep_frame(inp, fps_str):
        with torch.no_grad():
            frame, preds = inp
            return prep_display(args.claim_id,preds, frame, None, None, undo_transform=False, class_color=True, fps_str=fps_str)
...
def evaluate(net:Yolact,claimid, train_mode=False):
    ...
    #         evalimage(claimid,net, inp, out)
    #     else:
    #         evalimage(claimid,net, args.image)
    #     return
    # elif args.images is None:
    #     # inp, out = args.images.split(':')
    #     inp, out = f"{claimid}/",f"{claimid}/"
    #     print("inp, out >> ",inp, out)
    #     img_numpy = evalimages(claimid,net, inp, out)
    #     return
    inp, out = f"{claimid}/",f"{claimid}/"
    print("inp, out >> ",inp, out)
    evalimages(claimid,net, inp, out)
    # return
"""
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            if args.display:
                img_numpy = prep_display(claimid,preds, img, h, w)
...
#         evaluate(args.claim_id,net, dataset)

# trained_model="app/coin_mask_n.pth"
# config="coin_resnet50_config"
#####neww code#########
# trained_model="app/new_mask_coin.pth"
# config="coin_10_mask_config"
#3rd Model changes for coin Masking 10-01-2023############
trained_model="app/5RS_masking_30000.pth"
config="coin_5_mask_dataset"
##################
...
with torch.no_grad():
    ...
    torch.set_default_tensor_type('torch.cuda.FloatTensor')       
    dataset = None        
    print('Loading coin mask model...', end='')
    net = Yolact()
...
def coin_mask_func_5rs(claim_id):
    print("evaluate started calling")
    # print("net, dataset,claim_id >>>>> ",net, dataset,claim_id)
    evaluate(net, claim_id)
    with torch.no_grad():
